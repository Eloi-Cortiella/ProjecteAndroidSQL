...
interface EntradaBibliotecaDao {
    ...
    @Query(
        ...
        SELECT
            COUNT(*) AS total,
            SUM(CASE WHEN estat_lectura = :estatLlegit THEN 1 ELSE 0 END) AS llegits,
            SUM(CASE WHEN estat_lectura = :estatPerLlegir THEN 1 ELSE 0 END) AS per_llegir
    ...
    fun observarBiblioteca(
        idUsuari: Long,
        estat: String? = null
    ): kotlinx.coroutines.flow.Flow<List<com.app.projecteandroidsql.data.room.dao.LlibreBibliotecaItem>>
